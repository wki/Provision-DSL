...
### inspectors, installers or do-it-myself

### how to influence state-calculation
...
L<Entity|Provision::DSL::Entity> must report its state using 
C<<< $self->add_to_state('outdated') >>>. 
A typical state routine looks like this:
    ...
    before calculate_state => sub {
        ...
        $self->add_to__state('missing');
    };

If the main entity reports a C<missing> state, it will remain and can never
get improved by a role or other depending entity. Otherwise depending roles
may degrade a C<current> entity to C<outdated> if any of them is not current.
