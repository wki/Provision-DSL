...
The directory typically has the following layout:
    ...
    .provision_some_name/
        provision/                  -- everything needed to run the script
...
After all needed files have been collected, an rsync daemon is started
on the local machine using the config file from the temporary directory.
The rsync daemon supports two modules ("provision" and "resources") that
simply point to the two identically named directories inside the
temporary directory. This allows future expansions currently not supported.

The daemon listens to the port '2873' on the loopback interface. The port can
be customized in the "local" section of the config file.
...
into several lines and indented to be readable.

    ssh some_user@hostname
        export PERL5LIB='/tmp/provision_XXX'
        mkdir -p /tmp/provision_XXX
        rsync -r rsync://127.0.0.1:2873/provision /tmp/provision_XXX/
        perl /tmp/provision_XXX/provision.pl
...
every perl starting version 5.10.0.
