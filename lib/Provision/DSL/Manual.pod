...
where configuration file might look like

    {
        name => 'sample config',
        provision => 'path/to/provisioning_script.pl',
...
provisioning. The entities are evaluated in source order and usually there
is no dependency between them, except explicitly specified.
