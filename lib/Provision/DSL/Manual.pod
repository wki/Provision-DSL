...
modules only have minimal dependencies, all modules are pure perl and
run starting from Perl 5.10.

All required modules, resources and the provision script are packed into
a tar file, the tar file is appended to a extraction script (into the
__DATA__ section, to be precise) and the combination of both is pasted
to the remote server's Perl interpreter where it is unpacked and executed.
