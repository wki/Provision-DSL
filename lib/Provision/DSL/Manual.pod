...
and your provisioning script could be
    ...
    Service 'mysite_http_daemon' => {
        ...
        contents => Template('etc/mydaemon.tt', { vars => { ... } }),
    };
...
are evaluated in source order. Each entity decides on its own if it must 
C<<< create() >>>, C<<< change() >>> or C<<< remove() >>> itself.
...
file, the tar file is appended to an extraction script (base-64 encoded, into
...
directory and executed. The entire process only requires a single ssh
process to run. All other IO-Operations happen entirely on the target machine,
the ssh connection will compress its payload when possible.
